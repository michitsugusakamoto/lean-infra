...
resource "aws_appautoscaling_policy" "api_scale_up" {
  ...
  depends_on = [aws_appautoscaling_target.api_ecs_target[0]]
}
...
resource "aws_appautoscaling_policy" "api_scale_down" {
  ...
  depends_on = [aws_appautoscaling_target.api_ecs_target]
}

resource "aws_cloudwatch_metric_alarm" "api_cpu_high" {
  count = var.is_staging_and_staging_off ? 0 : 1

  alarm_name          = "autoscaling_api_cpu_utilization_high"
  ...
  alarm_actions = [aws_appautoscaling_policy.api_scale_up[0].arn]
}

resource "aws_cloudwatch_metric_alarm" "api_cpu_low" {
  count = var.is_staging_and_staging_off ? 0 : 1

  alarm_name          = "autoscaling_api_cpu_utilization_low"
  ...
  alarm_actions = [aws_appautoscaling_policy.api_scale_down[0].arn]
}
